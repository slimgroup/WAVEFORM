% 2.5D example of PDEfunc, comparing to the analytic Green's function in a constant medium
...
opts.lsopts = lsopts;
% Takes around 8200s to generate in serial, be warned
tic,D3d = PDEfunc(PDEopts.FORW_MODEL,v0*ones(model.n),Q,[],[],model,opts);
disp(['3D data generation time ' num2str(toc)]);
...
opts.use_knyq = false;
opts.nky = nky;
tic,D25d = PDEfunc_25D(PDEopts.FORW_MODEL, vc2D, Q, [], [],model,opts);
...
model_plot = model;
model_plot.xsrc = linspace(0,L(1),nsrc+1);
model_plot.xrec = linspace(0,L(1),nrec+1);
plot_opts = struct;
plot_opts.new_fig = true;
plot_opts.cax = [-1 1]*50;
plot_opts.func = @real;
PR = @(x) plot_data2d(x,model_plot,plot_opts);
plot_opts.func = @imag;
PI = @(x) plot_data2d(x,model_plot,plot_opts);
