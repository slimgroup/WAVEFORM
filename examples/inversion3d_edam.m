...
% use the 3D forward modelling + FWI code
...
%% Plot results
figure; imagesc(model.xrec,model.yrec,real(init_res)); title('Initial residual'); ax = caxis;
figure; imagesc(model.xrec,model.yrec,real(final_res)); title('Final residual'); caxis(ax);

slice3D(v,model,1000,[],2000); title('True model'); ax = caxis;
slice3D(v0,model,1000,[],2000); title('Initial model'); caxis(ax);
slice3D(vest,model,1000,[],2000); title('Inverted model'); caxis(ax);
